...
describe Map do
  it "creates a new Map" do
    world = Map.new(height: 25, width: 80)
  ...
  it "initializes the Text with the fill" do
    world = Map.new(height: 25, width: 80, fill: '#')
  ...
  it "should support adding to parent" do
    world = Map.new(height: 25, width: 80)
    group = Map.new(height: 5, width: 5)
  ...
  it "should suppport removing from parent" do
    world = Map.new(height: 25, width: 80)
    group = Map.new(height: 5, width: 0)
    group.add(parent: world, x: 10, y: 15)
    group.remove
  ...
  it "should render the children with the text" do
    world = Map.new(height: 25, width: 80, fill: '-')
    group = Map.new(height: 5, width: 5, fill: '#')
  ...
  it "should renders by depth for siblings" do
    world = Map.new(height: 25, width: 80, fill: '-')
    group = Map.new(height: 5, width: 5, fill: '&')
    shadow = Map.new(height: 5, width: 5, fill: '#')
  ...
  it "should bump if out of bounds if x < 0" do
    world = Map.new(height: 10, width: 10, fill: '-')
    group = Map.new(height: 5, width: 5, fill: '&')
  ...
  it "should not action if siblings do not collide" do
    world = Map.new(height: 10, width: 10, fill: '-')
    group1 = Map.new(height: 5, width: 5, fill: '&')
    group2 = Map.new(height: 5, width: 5, fill: '#')
